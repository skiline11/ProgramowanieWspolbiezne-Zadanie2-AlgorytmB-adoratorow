...
//	x = arg max(v) { W(u, v) : v in N(u) - T(u) && (W(u, v) > W(v, S(v).last)) };
	...
	//tutaj wazna jest kolejnosc w who_i_dont_adorate
...
pair<bool, unsigned long> make_suitor(unsigned int method, unsigned long v1, unsigned long v2, unsigned long &i) {
	// W tej chwili mam P(v1) i P(v2)
	...
	if(y.first == false) {
		std::lock(*mutex_on_vertex[v1], *mutex_on_vertex[v2]);
		y = Sv_last(method, v2);
		if(y.first == false) {
			i++;
		...
		mutex_on_vertex[v2]->unlock();
	}
	else if(y.first == true) {
		std::lock(*mutex_on_vertex[v1], *mutex_on_vertex[v2], *mutex_on_vertex[y.second]);
		pair<bool, unsigned long> y2 = Sv_last(method, v2);
		if(y == y2) {
			i++;
...
//		cout << "Uwaga : dla y.second = " << y.second << ", mamy " << (who_i_dont_adorate_next.find(y.second) != who_i_dont_adorate_next.end()) << endl;
		...
		mutex_on_vertex[v1]->unlock();
		mutex_on_vertex[v2]->unlock();
		mutex_on_vertex[y.second]->unlock();
	}
	return y;
}
	...
	while(not end_of_work) {
		...
		if(vertex_to_process.empty()) {
			...
			while(i <= db[v1] && who_i_dont_adorate.at(v1).empty() == false && end_of_loop == false) { // while i <= b(u) - db[u] i mam jeszcze jakiegos sąsiada ( N(u) != exhausted)
				// UWAGA : who_i_dont_adorate.at(v1) może zostać zmienione tylko przez v1 wiec nie musze robic lock
				...
				if(best.first == true) { // best != NULL
					pair<bool, unsigned long> whom_i_annulled = make_suitor(b_method, v1, best.second, i);
